...
class SettingConroller: BaseViewController,ParameterDelegate {
...
//        self.navBarHairlineImageView.hidden = false
    ...
    func initView(){
        if AppTools.isExisNSUserDefaultsKey("user"){
            mUser = AppTools.loadNSUserDefaultsClassValue("user") as? User
            lbName.text = mUser.userName
...
//            self.performSelectorOnMainThread(#selector(self.checkUpdateWithData(_:)), withObject: receiveStatusDic, waitUntilDone: false)
    ...
    @IBAction func changePhone(sender: AnyObject) {
        ...
        controller.phoneTag = phone
        controller.phoneTitle = "修改联系电话"
