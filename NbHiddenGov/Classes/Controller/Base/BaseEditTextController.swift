    ...
    func save(){
        if((self.delegate) != nil)
        {
            ...
            if phoneTag != nil { //额外添加业务逻辑
                ...
                parameters["userVo.userPhone"] = text
                NetworkTool.sharedTools.changePhone(parameters) { (login, error) in
                    if error == nil {
                        
                        self.showHint("联系电话修改成功！", duration: 2, yOffset: 0);
                        
                        //本地存储电话号码
                        if AppTools.isExisNSUserDefaultsKey("user"){
                            let user :User! = AppTools.loadNSUserDefaultsClassValue("user") as? User
                            user.phone = text
                            AppTools.setNSUserDefaultsClassValue("user", user)
                        }
                        
                        //跳转回上一个界面
                        self.delegate.passParams(text,key:"",indexPaths: self.indexPaths)
