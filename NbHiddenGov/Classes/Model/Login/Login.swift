...
class Login: BaseModel {
    ...
    init(json: JSON) {
        ...
        let entityJson = json["data"].dictionaryObject
        let cpyName = entityJson!["userCompany"] as? String ?? ""
        
        let phone = entityJson!["userPhone"] as? String ?? ""
        let factName = entityJson!["factName"] as? String ?? ""
        let userName = entityJson!["anwei"] as? String ?? ""

        self.user = User(cpyName: cpyName, phone: phone, factName: factName, userName: userName)
    }
