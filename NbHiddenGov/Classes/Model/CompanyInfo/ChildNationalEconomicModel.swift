...
class ChildNationalEconomicModel:BaseModel,NSCoding{
    // MARK: - 保存和获取所有分类
    static let ChildNationalEconomicModelKey = "ChildNationalEconomicModelKey"
    var code: String?
    var id: NSNumber?
     ...
     */
    class func savaModels(models: [ChildNationalEconomicModel])
    {
        let data = NSKeyedArchiver.archivedDataWithRootObject(models)
        NSUserDefaults.standardUserDefaults().setObject(data, forKey: ChildNationalEconomicModel.ChildNationalEconomicModelKey)
    }
    ...
    /**
     ...
     - returns: 分类数组或者nil
     */
    class func loadLocalModels() -> [ChildNationalEconomicModel]?
    {
        if let array = NSUserDefaults.standardUserDefaults().objectForKey(ChildNationalEconomicModel.ChildNationalEconomicModelKey)
        {
            return NSKeyedUnarchiver.unarchiveObjectWithData(array as! NSData) as? [ChildNationalEconomicModel]
        }
        return nil
    }
