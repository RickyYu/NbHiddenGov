...
class NationalEconomicModel:BaseModel,NSCoding{
    // MARK: - 保存和获取所有分类
    static let NationalEconomicModelKey = "NationalEconomicModelKey"
    var code: String?
    var id: NSNumber?
     ...
     */
    class func savaModels(models: [NationalEconomicModel])
    {
        let data = NSKeyedArchiver.archivedDataWithRootObject(models)
        NSUserDefaults.standardUserDefaults().setObject(data, forKey: NationalEconomicModel.NationalEconomicModelKey)
    }
    ...
    /**
     ...
     - returns: 分类数组或者nil
     */
    class func loadLocalModels() -> [NationalEconomicModel]?
    {
        if let array = NSUserDefaults.standardUserDefaults().objectForKey(NationalEconomicModel.NationalEconomicModelKey)
        {
            return NSKeyedUnarchiver.unarchiveObjectWithData(array as! NSData) as? [NationalEconomicModel]
        }
        return nil
    }
